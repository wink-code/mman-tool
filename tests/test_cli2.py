...
from myfind.core import find_matched_file_paths, MATCHED_FILE_NAME
...
def test_force(tmp_path):
    ...
    (tmp_path / "src" / MATCHED_FILE_NAME).write_text("apple/*\nbanana/*.py\norange/*.css\norange/*.html\norange/*.js")
    


    run(source_root=tmp_path/"src", target_root=tmp_path/"dst", force=True, verbose=True)
    ...
    assert set(p.name for p in (tmp_path / "dst" / "apple").iterdir()) == {f"file{i}" for i in range(1, 4)}
    assert set(p.name for p in (tmp_path / "dst" / "banana").iterdir()) == {"file1.py"}
    assert set(p.name for p in (tmp_path / "dst" / "orange").iterdir()) == {"file1.css", "file2.html", "file3.js"}
...
def test_debug(tmp_path):
    ...
    # print(result)
    assert set(result) == {'apple/file1', 'apple/file2', 'apple/file3', 'banana/file1.py', 'orange/file1.css', 'orange/file2.html', 'orange/file3.js'}
