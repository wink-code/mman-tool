...
from .core import bench_copy, gen_the_needed_files_list
from .meta import TARGET_FILES_PATTERN, VERSION


def run(source_root: Path, target_root: Path, *, force: bool, verbose: bool) -> None:
    ...
    if force or not targets_file.exists():
        ...
        lines = find_matched_file_paths(patterns, work_dir=source_root)

        with open(targets_file, "w", encoding="utf-8") as f:
            f.write("\n".join(lines) + "\n")

    files = gen_the_needed_files_list(work_dir=source_root, verbose=verbose)
